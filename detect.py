...
model=load_model('falldetectionmodel.h5')
...
cap = cv2.VideoCapture('videoplayback (1)_Trim.mp4') 
...
while cap.isOpened():
    ...
    if ret == True:
        ...
        for pair in POSE_PAIRS:
            ...
            if points[idFrom] and points[idTo]:
                cv2.line(bll, points[idFrom], points[idTo], colour, 3)
        ...
        cv2.imwrite('output/detectpics/'+str(cframe)+'.jpg',bll)
        bll=np.zeros((frame_height,frame_width,3), dtype="uint8")
        if cv2.waitKey(25) & 0xFF == ord('q'):
            break
        img = image.load_img('output/detectpics/'+str(cframe)+'.jpg', target_size=(300,300))
    ...
    #print(file)
        ...
        if classes[0]==0:
            print("Fall detected!")
            colour=(0,0,255)
            print("Frame no: ",cframe)
            cv2.putText(frame,'Fall Detected!',org,cv2.FONT_HERSHEY_SIMPLEX,0.75,(0,0,255),2,cv2.LINE_AA)
            cv2.imwrite('output/pics/'+str(cframe)+'.jpg',frame)
            #cv2.imshow(kwinName, frame)
            #break
        else:
            print("No fall detected in ",cframe)
            cv2.putText(frame,'No Fall Detected',org,cv2.FONT_HERSHEY_SIMPLEX,0.75, (0,255,0),2,cv2.LINE_AA)
            cv2.imwrite('output/pics/'+str(cframe)+'.jpg',frame)
            #cv2.imshow(kwinName, frame)
...
#model = load_model('C:/Users/Dell/Desktop/falldetectionnew/model/falldetectionmodel.h5')
#path='C:/Users/Dell/Desktop/falldetectionnew/data14'
